OATHBREAKERS - JOGO EM EXECUÇÃO

Neste repositório está a versão 5 do jogo Oathbreakers, favor ver versões anteriores para perceber como o jogo funciona:

https://github.com/HugoUAC/Oathbreakers---Entrega-Intermedia-V1-e-V2-.git

Nesta versão, o jogo já é possível ser executado mas em terminal apenas. 

Ao iniciar o servidor é criado aleatoriamente um mapa de jogo com um número de quadrículas fixo (12X10). Neste mapa existe paredes à volta para evitar que o jogador salte fora do mapa definido, ainda mais existe inimigos e baús. O mapa está definido da seguinte maneira:

"#" - Paredes<br/>
"%" - Inimigos<br/>
"$" - Baús (Ainda não implementado)<br/>
(A-Z) - Jogador (Primeira letra do nome do jogador)<br/>



